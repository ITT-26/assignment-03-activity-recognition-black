...
class DataPreprocessser:
    def __init__(self):
        pass

    def convert_gyro_data(self, recordings):
    ...
    def split_recordings_to_train_test(self, recordings):
    ...
    def create_time_windows(self, recordings, window_length):
        windows = []
    ...
    def transform_windows_to_features(self, windows):
        feature_rows = []
    ...
    def perform_standard_scaling(self, train_df, df_to_scale):
        ...
        df_scaled[[col for col in df_scaled.columns if col != "activity"]] = (
            scaled_samples
        )
        return df_scaled

    def perform_normalization(self, train_df, df_to_normalize):
        scaler = MinMaxScaler()
        scaler.fit(train_df[[col for col in train_df.columns if col != "activity"]])
        scaled_samples = scaler.transform(
            df_to_normalize[
                [col for col in df_to_normalize.columns if col != "activity"]
            ]
        )
