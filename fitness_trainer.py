...
class ActivityDisplay():
    ...
    def create_icons(self):
        ...
        self.confidence_icon = pyglet.sprite.Sprite(
            pyglet.image.load("img/circle-check.png"),
            batch=self.icons
        )
        self.confidence_icon.scale = 0.5
        self.confidence_icon.x = self.confidence_score_label.x - self.confidence_score_label.content_width / 2 - 50 - self.confidence_icon.width /2
        self.confidence_icon.y = self.confidence_score_label.y - 20
        
        self.time_icon = pyglet.sprite.Sprite(
            pyglet.image.load("img/stopwatch.png"),
            batch=self.icons
        )
    ...
    def get_activity_images(self, activity):
        ...
        try:
            self.activity_images = [pyglet.image.load(
                f1), pyglet.image.load(f2)]
    ...
    def on_draw(self):
        ...
        if self.activity_images:
            ...
            gl.glBlendFunc(gl.GL_SRC_ALPHA, gl.GL_ONE_MINUS_SRC_ALPHA)  # pylint: disable=unreachable
...
class Trainer():
    ...
    def update_display(self):
        self.activity_display.update_display(
            self.current_activity, self.confidence)

    def draw(self):
        self.background.blit(0, 0, width=self.window.width,
...
def main():
    ...
    pyglet.clock.schedule_interval(lambda dt: fake_data(dt, trainer), 1.0)


def fake_data(dt, trainer):
    activities = ["Running", "Rowing", "Lifting", "Jumping Jack"]
    activity = activities[int(time.time()) % len(activities)]
    confidence = 0.8
