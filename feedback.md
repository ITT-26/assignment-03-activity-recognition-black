# Team Black (16.5/20P)

## Gathering Training Data (5/6P)

* ok!
* DON'T COMMIT ALL THE TRAINING FILES, WE HAVE A SHARED REPO FOR THAT. I DO NOT HAVE UNLIMITED SPACE ON MY LAPTOP AND ALSO I DO NOT HAVE TIME TO WAIT TIL ALL THE DATA IS CLONED FOR EACH REPOSITORY...sorry for raising my voice but almost all of you did that *crying* (-1P)


## Activity Recognition (11.5/14P)

#### Evaluation

* very detailed analysis and documentation, well done!!
* very detailed comparison
* reasonable splits (LOSO vs. random)

#### Fitness Trainer

* had to search in the files for the path to the data (-1P)
* training on start and predicting continuously works fine
* activity recognition works most of the time but is very sensitive when doing nothing -> a lot of false positives (-1P)
* fitness application looks nice but there is no one who tells me what to do or for how long :o (-0.5P)


## Further Comments

For the future, please structure your files - do you know the concept of folders?